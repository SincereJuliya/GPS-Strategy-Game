"""
Magnets puzzle (simplified version) — a 4×4 grid divided into domino pairs.
Each domino contains a magnet (+/-) or is empty.
Rules:
- + and + cannot be neighbours
- - and - cannot be neighbours
- Each row/column has a required count of + and -.

Simplified implementation: we provide a ready solution and one hint,
the player must fill in the rest while respecting the rules.
"""
...
def _check_counts(grid, size, row_plus, row_minus, col_plus, col_minus):
    """Check that counts of + and - match the targets."""
    for r in range(size):
...
def generate() -> dict:
    ...
    size = 4
    # Predefined valid solution (hand-crafted)
    ...
    # Compute hints — counts of + and - in each row/column
    ...
    # Reveal 2–3 random hints on the board
...
def validate(puzzle_data: dict, user_solution) -> bool:
    """
    user_solution = {"grid": [[..]]} — full grid with "+", "-", "0".
    """
    ...
    # Neighbours cannot have identical poles
